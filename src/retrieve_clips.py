...
def topk_retrieval(args):
    ...
    np.save(os.path.join(args.feature_dir, 'sorted_indices.npy'), indices)
    print('Saving features to ...', args.feature_dir)
...
# This function doesn't belong to the actual code
# Only created to test retrieved videos for the demo
def retrieve_video(args,index=None):

    indices = np.load(os.path.join(args.feature_dir, 'sorted_indices.npy'))
    ks = [3]
    topk_correct = {k: 0 for k in ks}
    ...
    video_list = pd.read_csv ("data/ucf101/split/trainlist01.txt")
    predicted_videos=[]


    for k in ks:
        top_k_indices = indices[index, :k]
        for ind in top_k_indices:
            print(ind)
            video_sample = str(video_list.iloc[ind, 0]).split(" ")[0]
            predicted_videos.append(video_sample)
            print("Predicted : {} Retrieved Video name : {}".format(y_train[ind],video_sample))
...
def parse_args():
    ...
    parser.add_argument('--ckpt', type=str, help='checkpoint path', default="models/intraneg_r3d_res.pth")
    parser.add_argument('--bs', type=int, default=8, help='mini-batch size')
    parser.add_argument('--workers', type=int, default=8, help='number of data loading workers')
    parser.add_argument('--extract', default=False, type=ast.literal_eval, help='extract features when True')
    parser.add_argument('--modality', default='res', type=str, help='modality from [rgb, res, u, v]') 
    parser.add_argument('--merge', default=True, type=ast.literal_eval, help='If True, merge two input.')
...
if __name__ == '__main__':
    ...
    if args.extract: # make this true if you want to extract all feautres.
        extract_feature(args)
    topk_retrieval(args)
    # retrieve_video(args,index=204) This function can is just to test the retrived videos by giving the index. Not part of the actual code.
